...
import static io.openepcis.constants.ApplicationIdentifierConstants.NET_WEIGHT_AI_PARAM;
import static io.openepcis.constants.ApplicationIdentifierConstants.SGTIN_AI_URI_PREFIX;
/**
 * Additional class to validate the Non-EPCIS compliant DL identifiers: GTIN + Net Weight. Example: https://example.com/01/09520123456788?3103=000195
 * These types of are not supported by the EPCIS 2.0.0 specification but valid according to the GS1
 */
public class GTINWeightValidator implements ApplicationIdentifierValidator {
    private static final List<Matcher> DIGITAL_LINK_VALIDATION_RULES = new ArrayList<>();

    static {
        // GTIN + Net Weight Digital Link URI identifier validation rules
        ...
        // Validate for the Net Weight (3103)
        DIGITAL_LINK_VALIDATION_RULES.add(
                new Matcher("(http|https)://.*./01/[0-9]{14}\\?3103=(\\d{6})",
                        "Invalid Identifier, contains invalid Net Weight (30) (Ex: https://id.gs1.org/01/09520123456788/30/2.5). Please check the DL URI: %s"));
    }

    @Override
    public boolean supportsValidation(final String identifier) {
        return identifier.contains(SGTIN_AI_URI_PREFIX) && identifier.contains(NET_WEIGHT_AI_PARAM);
    }
    ...
    public boolean supportsValidation(final String identifier, final boolean isEpcisCompliant) {
        ...
        // else check if identifier contains DL URI part: "/01/", "?3103="
    ...
    public boolean validate(final String identifier, final boolean isEpcisCompliant, final Integer... gcpLength) {
        // if isEpcisCompliant is true then return false as the identifier is not EPCIS compliant
        // else if contains the DL URI part: "/01/", "3103" then validate the identifier
