        ...
        URN_MATCHERS.add(
                new Matcher(
                    ...
                    public void validate(final String urn) throws ValidationException {
                        super.validate(urn);
                        final String gcp = urn.substring(urn.indexOf(GRAI_AI_URN_PREFIX) + GRAI_AI_URN_PREFIX.length(), urn.indexOf('.'));
                        String grai;

                        if (urn.indexOf(".", urn.indexOf(".") + 1) == -1) {
                            grai = gcp + urn.substring(urn.indexOf('.') + 1);
                        } else {
                            grai = gcp + urn.substring(urn.indexOf('.') + 1, urn.indexOf(".", urn.indexOf(".") + 1));
                        }

                        if (grai.length() != 12) {
                            throw new ValidationException(String.format("Invalid GRAI Length, GRAI with Serial must be 14 digits followed by 1 to 16 alphanumeric characters (Ex: urn:epc:id:grai:1234567890.12.1ABC), Please check the provided URN: %s", urn));
                        }
                    }
                });
        URN_MATCHERS.add(
        ...
        URN_WITHOUT_SERIAL_MATCHERS.add(
                new Matcher(
                    ...
                    public void validate(final String urn) throws ValidationException {
                        ...
                        if (grai.length() != 12) {
                            throw new ValidationException(String.format("Invalid GRAI Length, GRAI with Serial must be 14 digits followed by 1 to 16 alphanumeric characters (Ex: urn:epc:idpat:grai:1234567890.12.*), Please check the provided URN: %s", urn));
                        }
                    }
                });
        ...
        URI_MATCHERS.add(
                new Matcher(
                    ...
                    public void validate(final String uri, final ValidationContext validationContext) throws ValidationException {
                        ...
                        CheckDigitValidator.validateGRAI(uri);
                    }
                });
